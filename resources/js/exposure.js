...
<path d="M70.109,69.973l-4.893,-4.88l-30.216,4.88"
    ...
    this.dir = element.value;
    this.angle = this.angles[this.dir];
...
this.init(element).then(()=>{
    ...
    element.addEventListener('focus', function (e) {
        //this.popover.removeAttribute('x-expcloak');
        this.addPopover();
        this.isOpen=true;
        this.onOpen=true;
        let inp = e.target;

        if (inp.value in this.angles ) {
            this.dir=inp.value;
            this.angle = this.angles[inp.value];
            this.drawArrow();
        }

        // close popover
        window.addEventListener('click', function (event) {
            if (event.target!=this.canvas && event.target!=this.element) {
                this.popover.remove();
                this.isOpen=false;
            }
        }.bind(this));
        ...
        // drag
        this.popover.addEventListener('mousedown', function (event) {
            if (this.isOpen) {
                ...
                inp.value = this.direction[this.angle];
                this.dir = this.direction[this.angle];
                this.drawArrow();
            }
        }.bind(this));

        this.popover.addEventListener('dblclick', function (event) {
            this.isDragged = false;
            this.popover.remove();
            this.isOpen=false;
        }.bind(this));
        ...
        window.addEventListener('mousemove', function (event) {
            ...
            if (event.target.matches(':hover')) {
                if (this.isDragged) {
                    ...
                    this.x = event.clientX - bounds.left;
                    this.y = event.clientY - bounds.top;
                    this.element.value = this.direction[this.angle];
                    this.dir = this.direction[this.angle];
                    this.drawArrow();
    ...
    this.addPopover = function () {
        ...
        if (this.element.nextSibling.tagName != 'DIV') {

            if (this.element.closest('.popwin')) {
                this.popover=this.element.closest('.popwin');
                return;
            }
            this.popover = document.createElement('div');
            this.popover.setAttribute('id', 'popover_' + this.id);
            this.popover.classList.add('sun');
            this.popover.classList.add('popwin');
            this.popover.classList.add('bottom');
            this.popover.classList.add('align-left');
            //this.popover.setAttribute('x-expcloak', '');
            this.popover.innerHTML = this.tpl;
            this.element.after(this.popover);
            this.canvas = this.element.nextSibling.querySelector('canvas');
            this.ctx = this.element.nextSibling.querySelector('canvas').getContext('2d');
            //this.ctx = document.getElementById('sun_' + this.id).getContext('2d');
            this.x0 = this.popover.offsetWidth / 2;
            this.y0 = this.popover.offsetHeight / 2;
            this.isOpen=true;
    ...
    this.drawArrow = function () {
       ...
       if (!(this.dir in this.angles))  return;
        ...
        this.x0 = this.popover.offsetWidth / 2;
        this.y0 = this.popover.offsetHeight / 2;
        this.ctx.clearRect(0, 0, this.popover.clientWidth, this.popover.clientHeight);
